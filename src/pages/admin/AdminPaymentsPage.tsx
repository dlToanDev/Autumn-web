...
export default function AdminPaymentsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        setError(e?.response?.data?.message || 'Không thể tải lịch sử thanh toán của admin.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      incomingAmount: sumPaymentAmount(incoming.filter((p) => isPaidStatus(p.status))),
      waitingIncomingAmount: sumPaymentAmount(incoming.filter((p) => !isPaidStatus(p.status))),
      outgoingAmount: sumPaymentAmount(outgoing.filter((p) => isPaidStatus(p.status))),
      waitingConfirmCount: payments.filter((p) => isWaitingConfirmStatus(p.status)).length,
  ...
  const openPayment = async (paymentCode: string) => {
    ...
    try {
      ...
      const e = err as { response?: { data?: { message?: string } } }
      setPaymentError(e?.response?.data?.message || 'Không thể mở chi tiết payment.')
    } finally {
      setOpeningCode('')
    }
  }
  ...
  const confirmPaymentByCode = async (paymentCode: string) => {
    ...
    setMessage('')
    try {
      const payment = await paymentApi.adminMarkPaid(paymentCode)
      setSelectedPayment((prev) => (prev?.paymentCode === payment.paymentCode ? payment : prev))
      setPayments((prev) => prev.map((p) => (p.paymentCode === payment.paymentCode ? payment : p)))
      setMessage('Đã xác nhận thanh toán thành công.')
    } catch (err: unknown) {
      const e = err as { response?: { data?: { message?: string } } }
      setPaymentError(e?.response?.data?.message || 'Không thể xác nhận payment.')
    } finally {
      setConfirmingCode('')
    }
  }
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="font-display text-2xl font-bold text-[#292524]">Thanh toán hệ thống</h1>
              <p className="mt-2 text-sm leading-6 text-[#78716C]">
                Theo dõi dòng tiền hoa hồng, các payment cần xác nhận và lịch sử giao dịch liên quan đến admin.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link to="/admin/commission-payments" className="inline-flex min-h-10 max-w-full flex-wrap items-center justify-center gap-2 rounded-[10px] bg-[#C96A3D] px-4 py-2 text-center text-sm font-medium leading-snug text-white shadow-sm transition-colors hover:bg-[#B85C38] hover:text-white">
                <Wallet size={16} />
                Thu hoa hồng
              </Link>
              <Link to="/admin/qr-settings" className="inline-flex min-h-10 max-w-full flex-wrap items-center justify-center gap-2 rounded-[10px] border border-[#E7D8C8] bg-white px-4 py-2 text-center text-sm font-medium leading-snug text-[#5B4636] transition-colors hover:bg-[#FAF6EF] hover:text-[#5B4636]">
                <QrCode size={16} />
                VietQR
              </Link>
              <Link to="/admin/payment-settings" className="inline-flex min-h-10 max-w-full flex-wrap items-center justify-center gap-2 rounded-[10px] border border-[#E7D8C8] bg-white px-4 py-2 text-center text-sm font-medium leading-snug text-[#5B4636] transition-colors hover:bg-[#FAF6EF] hover:text-[#5B4636]">
                <Settings size={16} />
                Cài đặt
              </Link>
          ...
          <div className="mb-5 rounded-[18px] border border-[#E8DED1] bg-[linear-gradient(135deg,#FFF8F3_0%,#FFFFFF_58%,#F3E7D3_100%)] p-5 shadow-[0_18px_48px_-32px_rgb(91_70_54/0.42)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#9A8775]">Tổng quan</p>
                  <h2 className="mt-1 font-display text-lg font-semibold text-[#2C241D]">{stats.total} payment đang ghi nhận</h2>
                  <p className="mt-1 text-sm text-[#78716C]">Ưu tiên các giao dịch chờ xác nhận trước khi đối soát hoa hồng.</p>
                </div>
              </div>
              <Badge variant={stats.waitingConfirmCount > 0 ? 'warning' : 'success'}>
                {stats.waitingConfirmCount > 0 ? `${stats.waitingConfirmCount} cần xác nhận` : 'Không có giao dịch chờ'}
              </Badge>
            </div>
          </div>
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-6">
            ...
            <StatCard title="Đã thu" value={formatCurrency(stats.incomingAmount)} icon={<TrendingUp size={20} />} color="accent" />
            <StatCard title="Chờ xác nhận" value={stats.waitingConfirmCount} icon={<Hourglass size={20} />} color="gold" />
            <StatCard title="Chưa thu xong" value={formatCurrency(stats.waitingIncomingAmount)} icon={<CreditCard size={20} />} color="secondary" />
            <StatCard title="Đã chi" value={formatCurrency(stats.outgoingAmount)} icon={<TrendingDown size={20} />} color="brown" />
          ...
          <Card padding="none" className="overflow-hidden">
            <div className="border-b border-[#E8DED1] px-5 py-4">
              <CardHeader className="mb-4 items-start">
                <div>
                  <CardTitle>Danh sách payment</CardTitle>
                  <p className="mt-1 text-sm text-[#78716C]">Lọc theo trạng thái, dòng tiền và loại payment để theo dõi lịch sử thanh toán.</p>
                </div>
            ...
            <PaymentHistoryTable
              ...
              emptyDescription={payments.length === 0 ? 'Hiện chưa có payment nào liên quan đến admin.' : 'Thử đổi bộ lọc để xem dữ liệu khác.'}
